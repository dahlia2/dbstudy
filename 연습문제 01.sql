...
CREATE TABLE MEMBER_TBL (   -- 테이블 새로 생성 시엔 괄호 붙여주기
       MEMBER_NO NUMBER             NOT NULL,
       ID        VARCHAR2(30 BYTE)  NOT NULL  UNIQUE,
       PW        VARCHAR2(30 BYTE)  NOT NULL,
       POINT     NUMBER,
       GRADE     VARCHAR2(10 BYTE)  CHECK (GRADE IN('VLP', 'GOLD', 'SILVER', 'BRONZE')),
...
ALTER TABLE MEMBER_TBL                 -- 하나 더할 때 ALTER 하나씩!
...
ALTER TABLE MEMBER_TBL   
    DROP COLUMN ADDRESS;    -- '칼럼' 붙여주고 칼럼명 적기


-- 4. 회원등급 칼럼의 타입을 VARCHAR2(20 BYTE)으로 수정하시오.
ALTER TABLE MEMBER_TBL  
      MODIFY GRADE VARCHAR2(20 BYTE);   -- 칼럼 타입 변경시 MODIFY


-- 5. 회원패스워드 칼럼의 이름을 PWD로 수정하시오.
ALTER TABLE MEMBER_TBL
     RENAME COLUMN PW TO PWD;       -- 칼럼 이름 변경시 RENAME 'COLUMN'
...
-- 9. 글내용 칼럼의 필수 제약조건을 제거하시오.  -- 필수 제약조건 제거 -> NOTNULL 제거 -> 기본값인 NULL로 변경 >> MODIFY
ALTER TABLE BOARD_TBL
    MODIFY CONTENT VARCHAR2(4000 BYTE) NULL;


-- 10. 글번호 칼럼에 기본키(PK_BOARD)를 설정하시오. -- 기본키 설정 >> 제약조건 추가 >> ADD CONSTRAINT
ALTER TABLE BOARD_TBL
...
ALTER TABLE BOARD_TBL   -- BOARD 테이블의 외래키 설정 -> BOARD 테이블 수정
    ADD CONSTRAINT FK_BOARD_MEMBER  -- 외래키_참고받는테이블_참고주는테이블
     FOREIGN KEY (WRITER) -- [외래키] (참고받는 칼럼)
        REFERENCES MEMBER_TBL(ID)  -- [REFERENCES (참조줄 기본키)] 참고주는테이블(참조주는 칼럼)
             ON DELETE CASCADE;  -- 기본키가 삭제되면 해당 참조값은 모두 삭제
