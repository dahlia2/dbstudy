...
3. 복사하는 쿼리
    ...
    2) 데이터를 제외하고 구조만 복사하기(칼럼만 복사하기)
        CREATE TABLE 테이블 AS(SELCET 칼럼 FROM  테이블 WHERE 1=2);
*/
...
CREATE TABLE EMPLOYEES
    ...
    FROM HR.EMPLOYEES
    WHERE 1=2);
    -- 굳이 WHERE 1=2 쓰는 이유가 문법적인 것 때문에?
*/
...
DECLARE
    EMP EMPLOYEES%ROWTYPE; -- 행 전체 타입을 가져오겠다
BEGIN
...
-- ==============================================================
DECLARE
    FNAME EMPLOYEES.EMPLOYEE.FIRST_NAME%TYPE;
BEGIN
   SELECT FIRST_NAME
     INTO FNAME
     FROM EMPLOYEES
