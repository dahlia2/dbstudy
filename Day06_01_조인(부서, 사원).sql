/* 
    ...
    2. 드리븐(DRIVEN) 테이블
      ...
      2) 행의(ROW)의 개수가 일반적으로 많고, FK를 조인 조건으로 사용하기 때문에 인덱스(INDEX) 활용이 불가능하다.
      3) 드라이브 테이블을 먼저 작성한다. 드리븐(DRIVEN) 테이블은 나중에 작성한다.
*/
...
-- 순서는 상관이 없지만 그래도 PK를 왼쪽에 기입하는 게 일반적!
...
-- 3. [오른쪽 외부] 조인(오른쪽에 있는 테이블은 일치하는 정보가 없어도 무조건 조인한다.)
 ...
 -- 1) 표준 문법 (WHERE 대신 ON)
SELECT D.DEPT_NO, D.DEPT_NAME, E.EMP_NO, E.NAME
  FROM EMPLOYEE_TBL E RIGHT OUTER JOIN DEPARTMENT_TBL D
    ON D.DEPT_NO = E.DEPART;
    
-- 2) 오라클 문법
SELECT D.DEPT_NO, D.DEPT_NAME, E.EMP_NO, E.NAME
  FROM DEPARTMENT_TBL D, EMPLOYEE TBL E
 WHERE D.DEPT_NO(+) = E.DEPART;  -- 반대편에 (+) 
 
-- =================================================================
 
-- 외래키 제약조건을 위반하는 데이터 삭제하기
DELETE FROM EMPLOYEE_TBL WHERE EMP_NO = 1005; -- PK를 조건으로 사용하면 인덱스를 타기 때문에 빠르다.
DELETE FROM EMPLOYEE_TBL WHERE EMP_NAME = '김성실'; -- PK 조건이 아닌 것을 사용하면 느리다.
