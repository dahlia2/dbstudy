...
DELETE
 ...
 WHERE USER_ID = (SELECT USER_ID
                    FROM USERS
                   WHERE USER_NO = 5);   -- 직접 참고되는 값이 없기 때문에 쿼리문 작성
DELETE
  FROM USER 
 WHERE USER_NO = 5;
...
-- 6. 제품을 구매한 사용자의 아이디별 구매횟수를 조회하시오.  -- 아이디별 -> '~별' 묶어야하니까 GROUP BY가 필요
-- 아이디  구매횟수
-- KHD     3
-- PSH     3
-- KYM     1
-- LHJ     2
SELECT USER_ID AS 아이디, COUNT(*) AS 구매횟수  -- 그룹바이는 꼭 함수를 사용함
  FROM BUYS
 GROUP BY USER_ID;  -- 아이디별 이니까 아이디로 묶어줘야 함!
...
SELECT U.USER_ID AS 아이디
 ...
 GROUP BY U.USER_ID, U.USER_NAME; -- 아디와 이름이 같다면 같은 사람으로 보고 묶어줌
...
SELECT U.USER_NAME AS 고객명
  ...
  FROM USERS U LEFT OUTER JOIN BUYS B
    ON U.USER_ID = B.USER_ID LEFT OUTER JOIN PRODUCTS P  -- 사용자 정보를 데이터를 계속 유지할 수 있도록 또 LEFT OUTER JOIN
...
SELECT U.USER_NAME AS 고객명
    , (SELECT P.PROD_NAME
         FROM PRODUCTS P
        WHERE P.PROD_CODE = B.PROD_CODE) AS 제품명
  FROM USERS U LEFT OUTER JOIN BUYS B
    ON U.USER_ID = B.USER_ID
 ORDER BY U.USER_ID;
