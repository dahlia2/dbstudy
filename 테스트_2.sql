...
SELECT BIRTHDATE AS 생일
  FROM STUDENT
 WHERE BIRTHDATE = TO_CHAR(BIRTHDATE, 'YYYY-MM-DD');
...
*/
SELECT RANK() OVER(ORDER BY SAL DESC) AS 연봉순위
     , PNAME AS 교수명
     , SAL AS 연봉
  FROM PROFESSOR
 ORDER BY SAL DESC;
...
*/
-- 사용자 함수 컴파일
CREATE OR REPLACE FUNCTION GET_STUDENT(USID STUDENT.USERID%TYPE)
RETURN NUMBER
IS
    STNO STUDENT.STUDNO%TYPE; 
BEGIN
    SELECT STUDNO
      INTO STNO
      FROM STUDENT
...
/*
12.
학생(STUDENT) 테이블의 어떤 행(Row)을 삭제(DELETE)하거나 수정(UPDATE)하면 '학생 데이터가 
삭제되거나 수정되었습니다.'라는 서버메시지가 나타나는 STUD_TRIG 트리거를 작성하시오.
...
*/
CREATE OR REPLACE PROCEDURE MY_PROC(DENO IN DEPARTMENT.DEPTNO%TYPE) 
IS  
BEGIN
    DELETE
