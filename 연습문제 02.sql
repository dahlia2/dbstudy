...
CREATE TABLE CUSTOMER_TBL (
    ...
    PHONE         VARCHAR2(20 BYTE)  -- 번호지만 다시(-) 도 들어가므로 문자 처리
);
...
CREATE SEQUENCE BOOK_SEQ
    NOCACHE;  -- 노캐시를 쓰는 이유는 메모리를 아끼기 위해서
    
-- BOOK_ID(책 번호)에 순번 주고자 시퀀스 사용
-- 시퀀스 추가는 데이터 추가 -> INSERT
-- INSERT INTO 테이블명(칼럼들) VALUES (테이블_SEQ.NEXTVAL함수)  -- NEXTVAL은 번호 지정해주는 함수
-- 순서대로 기입할 것
-- 시퀀스와 각 칼럼의 데이터까지 입력
...
CREATE SEQUENCE ORDER_SEQ
    NOCACHE;
-- 시퀀스 일렬번호 주는 것을 날짜로 하되 YYDDMM- 형식으로 지정 -> TO_CHAR을 통해 글씨로 변환
-- 오라클에서 ||은 AND
...
-- 기본키 추가는 제약조건 추가이고, 그건 테이블의 변경이니까 ALTER
...
-- ADD COSTRAINT FK_오더테이블_(추가할컬럼)커스터머 // 제약조건 이름
--         FOREIGN KEY(지정할참조키칼럼)
--         REFERENECES 제공할테이블명(칼럼명)
--         ON DELETE SET NULL 기본키가 삭제되면 참조키는 NULL 처리
        
ALTER TABLE ORDER_TBL
    ADD CONSTRAINT FK_ORDER_CUSTOMER FOREIGN KEY(CUSTOMER_ID)
        REFERENCES CUSTOMER_TBL(CUSTOMER_ID)
            ON DELETE SET NULL; -- CUSTOMER_ID를 NOTNULL 처리했는데 가능한..?
/*
ALTER TABLE CUSTOMER_TBL
MODIFY CUSTOMER_ID NUMBER(11) NULL;
*/
-- ADD CONSTRAINT FK_이부분 단순 제약조건 이름 짓는 건지 아니면 FK지정받을테이블명_지정주는테이블명 규칙인 건지
-- CUSTOMER_ID 위에 기본키 설정해도 없다고 오류뜸 ..

ALTER TABLE ORDER_TBL
    ADD CONSTRAINT FK_ORDER_BOOK FOREIGN KEY(BOOK_ID)
        REFERENCES BOOK_TBL(BOOK_ID)
            ON DELETE SET NULL; -- 이또한 낫널..
...
-- 조회는 SELECT!
-- 김연아 데이터는 CUSTOMER 테이블에 있음 -> 별명 C로 지칭
-- 구매한 내역을 보기 위해 ORDER 테이블(별명 O로 지칭)과 교집합 -> INNER JOIN 

-- 표준문법
SELECT C.CUSTOMER_NAME AS 고객명 -- 조회된 칼럼명은 고객명이라 변경
     , COUNT(*) AS 구매도서수
  FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID -- 같은 데이터를 가진 기본키끼리 연결 -> 기본키와 외래키 연결
 WHERE C.CUSTOMER_NAME = '김연아'  -- 고객 이름 '김연아' 데이터 찾기
 GROUP BY C.CUSTOMER_ID, C.CUSTOMER_NAME; -- 고객 ID와 이름을 같이 묶어서 봄!
 
-- 오라클문법
SELECT C.CUSTOMER_NAME AS 고객명
     , COUNT(*) AS 구매도서수
     -- 모든 참조 칼럼의 수? .. 정확한 기능 다시 check
     -- 아마 교집합의 갯수 (오더 테이블에 있는 해당 회원 데이터 갯수)
...
-- 표준문법
SELECT C.CUSTOMER_NAME AS 고객명
  FROM CUSTOMER_TBL C LEFT OUTER JOIN ORDER_TBL O  -- ORDER 테이블엔 존재하지 않는 CUSTOMER(LEFT) 의 데이터 찾기! LEFT에 있는 OUTER
    ON C.CUSTOMER_ID = O.CUSTOMER_ID
 WHERE O.ORDER_ID IS NULL -- 주문내역이 없으니 오더테이블 아이디 값이 NULL 
 GROUP BY C.CUSTOMER_ID, C.CUSTOMER_NAME;
 
-- 오라클 (+ NULL 말고 0으로 다른 기법으로 조회)
SELECT C.CUSTOMER_NAME AS 고객명
  FROM CUSTOMER_TBL C, ORDER_TBL O
 WHERE C.CUSTOMER_ID = O.CUSTOMER_ID(+) 
 GROUP BY C.CUSTOMER_ID, C.CUSTOMER_NAME
...
-- 박지성이 도서를 구매했고 -> CUSTOMER와 ORDER를 이어주고
-- 그 주문한 도서의 출판사의 개수 -> CUSTOMER와 ORDER의 교집합에서 BOOK와 교집합인 것

-- 표준문법
SELECT C.CUSTOMER_NAME AS 고객명
     , COUNT(DISTINCT B.PUBLISHER) AS 출판사수  -- PUBLISHER을 왜 제외하는지...
  FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID -- 먼저 기본키와 참조키 이어준 것을
        INNER JOIN BOOK_TBL B -- 퍼블리쉬 정보가 들어있는 BOOK 테이블과 이어준다
            ON O.BOOK_ID = B.BOOK_ID -- 그리고 그 둘의 기본키와 참조키를 이어줌
...
-- 고객과 주문과 책 테이블이 필요하고
-- 주문테이블의 오더테이블 칼럼에서 조건을 제외한 걸 조회
...
WHERE TO_DATE(O.ORDER_DATE, 'YY/MM/DD') -- 주문날짜 조회
      NOT BETWEEN TO_DATE('20/07/04', 'YY/MM/DD') -- 포함하지 않는다 20년 7월 4일부터
              AND TO_DATE('20/07/07', 'YY/MM/DD'); --20년 7월 7일까지
