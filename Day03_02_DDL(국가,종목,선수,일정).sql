...
-- 기본키 제거하기 (문법 소개일 뿐, 실제로 작성할 필요는 없음. DROP TABLE을 수행하면 제약조건도 모두 제거된다.)
...
-- 외래키 제거하기 (문법 소개일 뿐, 실제로 작성할 필요는 없음. DROP TABLE을 수행하면 제약조건도 모두 제거된다.
ALTER TABLE PLAYER_TBL
...
-- 외래키 추가하기
ALTER TABLE PLAYER_TBL
    ADD CONSTRAINT FK_PLAYER_NATION FOREIGN KEY(N_NODE)
    REFERENCES NATION_TBL (N_CODE)
    ON DELETE CASCADE;
    
ALTER TABLE PLAYER_TBL
    ADD CONSTRAINT FX_PLAYER_EVENT FOREIGN KEY(E_CODE)
