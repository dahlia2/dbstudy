/*
    프로시저(PROCEDURE)  -> 과정을 묶어서 메소드처럼 만드는 것이라고 생각하면 됨!
    1. 하나의 프로시저에 여러 개의 쿼리문을 작성해서 처리할 수 있다.
    2. 여러 개의 쿼리문이 필요한 서비스를 프로시저로 작성해 두면 사용이 편리하다.
        (예시 : 은행 이체(UPDATE 쿼리가 2개로 구성))
    3. 형식
        CREATE [OR REPLACE] PROCEDURE 프로시저명[(매개변수)]  -> 매개변수가 없을 때엔 괄호 자체를 생략할 수 있음
        IS -- AS도 가능
          변수선언
        BEGIN
            본문
...
DECLARE -- 프로시저는 변수가 꼭 있어야 하기 때문에 변수 선언을 따로 하였음
    FNAME EMPLOYEES.FIRST_NAME%TYPE;
BEGIN
    PROC5(100, FNAME);
...
CREATE TABLE PROD_TBL (
    /* 제품번호 */ P_CODE  NUMBER NOT NULL,    
    /* 제품이름 */ P_NAME  VARCHAR2(20 BYTE),
    /* 제품가격 */ P_PRICE NUMBER,
    /* 제품재고 */ P_STOCK NUMBER
);

-- 시퀀스 생성
CREATE SEQUENCE PROD_TBL_SEQ
    NOCACHE;

-- 데이터 추가
ALTER TABLE PROD_TBL
    ADD CONSTRAINT PK_PROD PRIMARY KEY(P_CODE);
INSERT INTO PROD_TBL(P_CODE, P_NAME, P_PRICE, P_STOCK) VALUES(PROD_TBL_SEQ.NEXTVAL, '홈런볼', 1000, 100);
INSERT INTO PROD_TBL(P_CODE, P_NAME, P_PRICE, P_STOCK) VALUES(PROD_TBL_SEQ.NEXTVAL, '맛동산', 2000, 100);
COMMIT;

-- 고객 테이블 구성하기
CREATE TABLE CUST_TBL (
    /* 고객번호 */   C_NO NUMBER NOT NULL,
    /* 고객이름 */   C_NAME VARCHAR2(20 BYTE),
    /* 고객포인트 */ C_POINT NUMBER
);
...
ALTER TABLE BUY_TBL
    ...
    ON DELETE SET NULL; -- 미리 뽑아보는 정보가 날아간다. 메모리를 아낄 수 있음
...
BEGIN
    ...
    -- 1) 구매내역 테이블에 구매 내역을 추가(INSERT)한다.
    INSERT INTO BUY_TBL(B_NO, C_NO, P_CODE, B_AMOUNT) VALUES(22, CNO, PCODE, BUY_AMOUNT);
